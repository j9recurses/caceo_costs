class Activity
  ...
  def self.activity_since(time)
    ...
    SURVEYS.each do |klass|
      ...
      result.concat klass.select("MAX(#{model_name}.id), '#{model_name}' AS model_name, county, #{model_name}.updated_at, election_years.year AS election_name, election_years.election_dt AS election_date, category_descriptions.name AS survey_name")
        .where("#{model_name}.updated_at > ?", time)
        .group(:county, :election_year_id)
        .joins("INNER JOIN election_years ON election_years.id = #{model_name}.id")
        .joins("INNER JOIN category_descriptions ON category_descriptions.model_name = '#{model_name}'")
    end
    result
  end

  def self.all_activity
    result = []
    SURVEYS.each do |klass|
      model_name = klass.to_s.downcase.pluralize
      result.concat klass.select("MAX(#{model_name}.id), '#{model_name}' AS model_name, county, #{model_name}.updated_at, election_years.year AS election_name, election_years.election_dt AS election_date, category_descriptions.name AS survey_name")
  ...
  def self.sort_by_county_election(array)
    ...
    grouped.map do |county_id, surveys|
      ...
      survey_hash.each do |elec, survs|
        survey_hash[elec] = survs.sort {|x,y| x.survey_name <=> y.survey_name }
      end
